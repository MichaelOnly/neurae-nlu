...
model = ElectraForSequenceClassification.from_pretrained("bhadresh-savani/electra-base-emotion", num_labels=17, ignore_mismatched_sizes=True)
...
sweep_config = {
   ...
   'parameters': {
      ...
      'scheduler_type': { 'values': ['linear', 'cosine', 'cosine_with_restarts', 'polynomial', 'constant', 'constant_with_warmup', 'inverse_sqrt']},
      'weight_decay': {'min': 0.0, 'max': 0.1}
   }
}
sweep_id = wandb.sweep(
      sweep_config, project="electra-sweep")
...
def train_with_wandb(config=None):
   with wandb.init(config=config):
      ...
      training_args = TrainingArguments(
            ...
            output_dir="models/electra",
            overwrite_output_dir=True,
            logging_strategy="steps",
            logging_dir="models/electra/logs",
            logging_steps=100,
